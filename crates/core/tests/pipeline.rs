...
// ── rust-static evaluator pipeline integration ────────────────────────────────

fn rust_static_registry() -> ComponentRegistry {
    use tgraphy_core::component::builtin::RustStaticEvaluator;
    let mut r = ComponentRegistry::new();
    r.register_evaluator("rust-static", Box::new(RustStaticEvaluator));
    r
}

fn rust_fixture(name: &str) -> std::path::PathBuf {
    std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
...
#[test]
fn rust_static_evaluator_resolves_by_name_in_registry() {
    let registry = rust_static_registry();
    let result = registry.resolve_evaluator("rust-static");
    assert!(
        result.is_ok(),
        "rust-static evaluator should resolve from registry"
    );
}

#[test]
fn rust_static_evaluate_step_produces_assessed_module_evidence() {
    let registry = rust_static_registry();
    let input = rust_fixture("assert_predicate_only.json");
    let output = temp_path("rust_static_assessed.json");

    evaluate_step(&registry, "rust-static", &input, &output)
        .expect("evaluate_step with rust-static should succeed");

    let artifact = tgraphy_core::io::read_artifact(&output).expect("output should be readable");
    let ArtifactKind::AssessedModuleEvidence(assessed) = artifact else {
        panic!("rust-static evaluate_step should produce assessed_module_evidence");
    };
    assert_eq!(assessed.assessment_layers.len(), 1);
    assert_eq!(assessed.assessment_layers[0].evaluator.id, "rust-static");
    assert_eq!(assessed.assessment_layers[0].findings.len(), 1);
    assert_eq!(
        assessed.assessment_layers[0].findings[0].rule_id.as_deref(),
...
#[test]
fn rust_static_evaluate_step_chains_from_assessed_module_evidence() {
    let registry = rust_static_registry();
    let input = rust_fixture("chaining_input.json");
    let output = temp_path("rust_static_chained.json");

    evaluate_step(&registry, "rust-static", &input, &output)
        .expect("evaluate_step chaining should succeed");

    let artifact = tgraphy_core::io::read_artifact(&output).expect("output should be readable");
    let ArtifactKind::AssessedModuleEvidence(assessed) = artifact else {
        panic!("chained evaluate_step should produce assessed_module_evidence");
    };
    assert_eq!(
        assessed.assessment_layers.len(),
        2,
        "chaining should preserve prior layer and append rust-static layer"
    );
    assert_eq!(assessed.assessment_layers[0].id, "prior-layer-001");
    assert_eq!(assessed.assessment_layers[1].evaluator.id, "rust-static");
