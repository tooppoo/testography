...
test:
		...
		cargo llvm-cov report --codecov --output-path cov.json \
				--ignore-filename-regex 'component/builtin/(evaluator|reporter)|cli/src/main|parser-rust/src/main|validation/schema'
		...
		# cli/src/main and parser-rust/src/main are excluded: both are thin binary entry
